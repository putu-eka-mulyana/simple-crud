...
require 'config.php';
$id =$_GET['id'];
$data= show("SELECT * FROM tbl_sinopsis WHERE id=$id")[0];
...
if(isset($_POST['simpan'])){
    ...
    if(!empty($nim && $clnd && $judul && $latar_belakang && $tujuan)){
        $result =insert("UPDATE tbl_sinopsis SET 
            nim='$nim',
            calon_dosen_pembimbing='$clnd',
            judul='$judul',
                ...
                <div class="card-body">
                    <form action="" method="post">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="nim" placeholder="Nim" name="nim" value="<?=$data[1]?>"><span class="error"><?=$nimerr?></span>
                        </div>
                        <div class="form-group">
                            <label for="nama">calon desen pembimbing</label>
                            <input type="text" class="form-control" id="nama" name="calon_dosen_pembimbing" 
                                   placeholder="Nama dosen" value="<?=$data[2]?>"><span class="error"><?=$clnderr?></span>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputPassword1">Judul</label>
                            <input type="text" class="form-control" id="exampleInputPassword1" name="judul" 
                                   placeholder="judul" value="<?=$data[3]?>"><span class="error"><?=$judulerr?></span>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputPassword1">latar belakang</label>
                            <textarea class="form-control" id="exampleFormControlTextarea1" name="latar_belakang" rows="3"><?=$data[4]?></textarea>
                            <span class="error"><?=$latar_belakangerr?></span>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputPassword1">Tujuan</label>
                            <textarea class="form-control" id="exampleFormControlTextarea1" name="tujuan"rows="3"><?=$data[5]?></textarea><span class="error"><?=$tujuanerr?></span>
                        </div>
